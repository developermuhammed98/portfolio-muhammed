...
export default function Home() {
  ...
  return (
    <div className="relative min-h-screen z-10 flex flex-col pb-10">
      {/* Dynamic Header with Language Switcher */}
      <header className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center relative z-30">
        <div className="font-mono text-sm tracking-wider text-cyan-300 font-bold select-none">
          M.B. // dev
        </div>
        <div className="flex bg-[#0b122a]/90 border border-white/10 rounded-full p-0.5 shadow-lg select-none">
          <button
            onClick={() => toggleLang("tr")}
            className={`px-3 py-1 font-mono text-[11px] rounded-full transition-all cursor-pointer font-bold ${
              lang === "tr"
      ...
      <div className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row gap-6 lg:gap-8 xl:gap-12 relative z-10">
        ...
        {/* COLUMN 1: LEFT PORTRAIT COLUMN */}
        <div className="order-1 lg:order-1 w-full lg:w-[320px] xl:w-[400px] lg:h-[80vh] lg:sticky lg:top-16 flex lg:items-start lg:justify-start justify-center pt-4 pb-4 lg:pt-12 lg:pl-0 lg:-ml-5 xl:-ml-10 shrink-0">
        ...
        <div className="order-3 lg:order-2 flex-1 min-w-0 py-6 lg:py-12 flex flex-col gap-24 lg:gap-32">
            ...
            <div className="flex flex-col gap-5">
              <div className="glass-card p-6 lg:p-8 relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col gap-4 text-slate-100 text-sm sm:text-base leading-relaxed lg:pl-6">
                  ...
                  <div className="mt-4 pt-4 border-t border-white/[0.03] font-mono text-xs text-slate-350 flex flex-wrap gap-3 items-center">
                    <span>{portfolioData.stats}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="glass-card p-5 flex flex-col gap-3 min-h-[180px]">
                  <h3 className="font-mono text-xs sm:text-sm text-slate-200 font-semibold">// {portfolioData.educationTitle}</h3>
                ...
                <div className="glass-card p-5 flex flex-col justify-between gap-4 min-h-[180px]">
                  <div>
                    <h3 className="font-mono text-xs sm:text-sm text-slate-200 font-semibold">// {portfolioData.certificationTitle}</h3>
          ...
          <section id="skills" className="section-animate">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {portfolioData.skills.map((category, idx) => {
                ...
                return (
                  <div key={idx} className="glass-card p-5 flex flex-col gap-3.5 min-h-[160px]">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {category.tags.map((tag, tIdx) => (
                        <span
                          key={tIdx}
                          className={`font-mono text-[11px] text-slate-100 bg-white/[0.04] border border-white/[0.12] px-2 py-0.5 rounded-full transition-all duration-200 cursor-default ${tagHover}`}
                        >
                          {tag}
                        </span>
                      ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {portfolioData.projects.map((project, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`glass-card p-5 sm:p-6 flex flex-col justify-between gap-4 relative group overflow-hidden ${
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-3 min-h-0">
                      ...
                      <div className="min-h-0">
                        <h3 className="text-base sm:text-lg font-bold text-slate-50 group-hover:text-white transition-colors">
                          {project.title}
                        </h3>
                      ...
                      {isFeatured && project.highlights && (
                        <ul className="mt-1 flex flex-col gap-1.5">
                          {project.highlights.map((hl, hlIdx) => (
                            <li key={hlIdx} className="flex gap-2 items-start text-xs sm:text-sm text-slate-200">
                              <span className="text-cyan-300 font-mono text-[10px] mt-0.5">&gt;</span>
                              <span>{hl}</span>
                            </li>
                          ))}
                        </ul>
                    ...
                    <div className="flex flex-col gap-3 pt-3 border-t border-white/[0.03] mt-auto">
                      <div className="flex flex-wrap gap-1">
                        {project.tech.map((t, tIdx) => (
                          <span
                            key={tIdx}
                            className="font-mono text-[9px] sm:text-[10px] text-slate-200 bg-white/[0.04] border border-white/[0.12] px-2 py-0.5 rounded-full"
                          >
                            {t}
                          </span>
                        ))}
                      ...
                      <div className="flex flex-wrap items-center justify-end gap-1.5 font-mono text-[10px] select-none">
                        {project.github && (
                          <a
                            href={project.github}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="px-2 py-1 bg-white/[0.04] text-slate-200 hover:text-white border border-white/[0.12] hover:border-white/[0.22] rounded-md transition-all duration-200 flex items-center gap-1"
                          >
                            <span>GitHub</span>
                            <span className="text-slate-450 text-[8px]">↗</span>
                          </a>
                        )}
                        {project.url && (
                          <>
                            <button
                              ...
                              className={`px-2 py-1 rounded-md border transition-all duration-200 cursor-pointer flex items-center gap-1 font-semibold ${
                                accent === "violet" 
                                  ? "bg-violet-500/20 text-violet-200 border-violet-500/55 hover:bg-violet-500/30 hover:text-white"
                                  : accent === "indigo"
                                  ? "bg-indigo-500/20 text-indigo-200 border-indigo-500/55 hover:bg-indigo-500/30 hover:text-white"
                                  : "bg-cyan-500/20 text-cyan-200 border-cyan-500/55 hover:bg-cyan-500/30 hover:text-white"
                              }`}
                            >
                              <span>{portfolioData.ui.previewBtn}</span>
                            ...
                            <a
                              ...
                              className="px-2 py-1 bg-white/[0.04] text-slate-200 hover:text-white border border-white/[0.12] hover:border-white/[0.22] rounded-md transition-all duration-200 flex items-center gap-1"
                            >
                              <span>{portfolioData.ui.liveBtn}</span>
                              <span className="text-slate-450 text-[8px]">↗</span>
                            </a>
                          </>
                        )}
              ...
              {portfolioData.experience.map((exp, idx) => {
                ...
                return (
                  <div key={idx} className="relative group">
                    <span className={`absolute -left-[30px] top-6 w-2 h-2 rounded-full border border-[#020617] transition-all duration-300 ${
                      isSec 
                        ? "bg-slate-500 group-hover:bg-slate-350 shadow-[0_0_5px_rgba(148,163,184,0.3)]"
                        : "bg-cyan-300 group-hover:bg-cyan-200 shadow-[0_0_8px_#22d3ee]"
                    }`} />

                    <div className={`glass-card p-5 flex flex-col gap-2.5 ${isSec ? "opacity-80 hover:opacity-100" : ""} min-h-[140px]`}>
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                        ...
                        <div className="font-mono text-[10px] text-slate-400 sm:text-right shrink-0">
                          <div>{exp.period}</div>
                          {exp.location && <div className="text-slate-450 mt-0.5">{exp.location}</div>}
                        </div>
                      </div>
                      <p className="text-slate-200 text-xs sm:text-sm leading-relaxed mt-1">
                        {exp.description}
                      </p>
                    </div>
                  </div>
          ...
          <section id="contact" className="section-animate max-w-lg mx-auto w-full text-center flex flex-col gap-5 items-center justify-center py-6 min-h-[40vh]">
            <div className="flex flex-col gap-1.5">
              ...
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white tracking-tight">
                {portfolioData.ui.workTogetherHeadline}
              </h2>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full mt-2 select-none">
              ...
              >
                {portfolioData.contact.sendEmailText}
              </a>
            ...
            <span className="font-mono text-xs text-slate-400 mt-4 select-none">
              {portfolioData.ui.referencesLong}
            </span>
          </section>

        </div>

        {/* COLUMN 3: RIGHT STICKY INFO PANEL */}
        <aside className="order-2 lg:order-3 w-full lg:w-[280px] xl:w-[320px] lg:h-[80vh] lg:sticky lg:top-16 flex flex-col justify-between py-6 lg:py-12 border-b lg:border-b-0 lg:border-l border-white/[0.02] lg:pl-6 xl:pl-8 shrink-0">
          <div className="flex flex-col gap-6 lg:gap-8">
            <div className="text-center lg:text-left flex flex-col gap-3">
              ...
              <div className="flex flex-wrap justify-center lg:justify-start gap-1.5 mt-1">
                <span className="font-mono text-xs text-cyan-300 bg-cyan-500/10 border border-cyan-500/20 px-2.5 py-0.5 rounded-full select-none font-semibold">
                  React.js
                </span>
                <span className="font-mono text-xs text-violet-300 bg-violet-500/10 border border-violet-500/20 px-2.5 py-0.5 rounded-full select-none font-semibold">
                  Next.js
                </span>
                <span className="font-mono text-xs text-indigo-300 bg-indigo-500/10 border border-indigo-500/20 px-2.5 py-0.5 rounded-full select-none font-semibold">
                  Laravel
                </span>
              </div>

              <p className="text-xs sm:text-sm text-slate-200 leading-relaxed max-w-xs mx-auto lg:mx-0 mt-1.5">
                {portfolioData.bio}
              </p>

              {portfolioData.availableForWork && (
                <div className="flex items-center gap-2 text-xs font-mono text-emerald-300 bg-emerald-500/15 border border-emerald-500/30 px-3 py-1.5 rounded-full w-fit mx-auto lg:mx-0 select-none font-semibold">
                  ...
                  {portfolioData.ui.availableForWork}
                </div>
              )}
            </div>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center transition-all duration-200 group font-mono text-xs lg:text-sm cursor-pointer ${
                      isActive
                        ? "text-cyan-300 border-b lg:border-b-0 lg:border-l-2 border-cyan-300 px-1 lg:px-0 lg:pl-4 font-bold"
      ...
      {/* VS Code Status Bar */}
      <footer className="hidden lg:flex fixed bottom-0 left-0 right-0 h-[22px] bg-[#0f1123]/92 border-t border-[rgba(99,102,241,0.15)] backdrop-blur-sm font-mono text-[11px] text-slate-550 px-3 items-center justify-between z-50 select-none">
        <div className="flex items-center gap-3">
          <span className="hover:text-slate-355 transition-colors cursor-default">⎇ main</span>
          <span className="text-slate-700">·</span>
          <span className="hover:text-slate-355 transition-colors cursor-default">✓ TypeScript</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="hover:text-slate-355 transition-colors cursor-default">UTF-8</span>
          <span className="text-slate-700">·</span>
          <span className="hover:text-slate-355 transition-colors cursor-default">Next.js</span>
          <span className="text-slate-700">·</span>
          <span className="hover:text-slate-355 transition-colors cursor-default">Tailwind CSS</span>
        </div>
      </footer>
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 bg-[#0c0f20]/90 border-b border-white/[0.04] select-none font-mono">
              <div className="flex items-center gap-2">
                <button 
                  ...
                  title={portfolioData.ui.closeBtn}
                >
                  ✕
                </button>
              ...
              <div className="flex-1 max-w-lg mx-4 bg-[#030510]/80 border border-white/[0.05] rounded-lg px-3 py-1 text-[11px] text-slate-400 flex items-center justify-between gap-2 overflow-hidden truncate">
                ...
                <button 
                  ...
                  title={portfolioData.ui.reloadBtn}
                >
                  ↻
                </button>
              </div>
            ...
            <div className="bg-amber-500/10 border-b border-amber-500/15 px-4 py-2 text-[10px] sm:text-xs text-amber-400 font-mono flex items-center justify-between gap-4 select-none">
              <span>{portfolioData.ui.iframeWarning}</span>
              <a 
                href={activePreviewUrl} 
                target="_blank" 
                rel="noopener noreferrer" 
            ...
            <div className="flex-1 w-full bg-slate-950 relative">
              {isLoadingPreview && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#090b16]/95 z-10 gap-3 font-mono">
                  ...
                  <span className="text-[11px] text-slate-450">{portfolioData.ui.loading}: {activePreviewTitle}...</span>
                </div>
              )}
              <iframe
                id="preview-iframe"
